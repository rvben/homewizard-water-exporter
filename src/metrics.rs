use prometheus::{
    Counter, Gauge, GaugeVec, Opts, Registry, TextEncoder, Encoder
};
use anyhow::Result;
...
impl Metrics {
    pub fn new() -> Result<Self> {
        ...
        // Water consumption metrics
        let total_water = Counter::with_opts(
            Opts::new("homewizard_water_total_m3", "Total water consumption in m³")
        )?;
        registry.register(Box::new(total_water.clone()))?;
        
        let active_flow = Gauge::with_opts(
            Opts::new("homewizard_water_active_flow_lpm", "Current water flow in liters per minute")
        )?;
        registry.register(Box::new(active_flow.clone()))?;
        
        let water_offset = Gauge::with_opts(
            Opts::new("homewizard_water_offset_m3", "Water meter offset in m³")
        )?;
        registry.register(Box::new(water_offset.clone()))?;
        
        // Network metrics
        let wifi_strength = Gauge::with_opts(
            Opts::new("homewizard_water_wifi_strength_percent", "WiFi signal strength percentage")
        )?;
